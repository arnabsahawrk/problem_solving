/*   Author: Arnab Saha  Date: 09/03/2025 [20:57:12]   */
#include <bits/stdc++.h>
...
{
    seg[node] = (1LL * seg[node] * lazy[node]) % mod;

    if (begin != end)
    {
        int left = 2 * node, right = 2 * node + 1;
        lazy[left] = (1LL * lazy[left] * lazy[node]) % mod;
        lazy[right] = (1LL * lazy[right] * lazy[node]) % mod;
    }
...
ll query(int node, int begin, int end, int l, int r)
{
    ...
    int mid = begin + (end - begin) / 2, left = 2 * node, right = 2 * node + 1;
    return (query(left, begin, mid, l, r) + query(right, mid + 1, end, l, r)) % mod;
}
